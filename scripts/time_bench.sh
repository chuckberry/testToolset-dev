...
MIN=`echo $STATS | awk -v "col=$MIN_COL" '{print $(col)}'`
MAX=`echo $STATS | awk -v "col=$MAX_COL" '{print $(col)}'`
...
traceplotgif.sh "$DATA_FOLDER/$PERC_FILE" "$PNG_FOLDER/$IMG_PERC_FILE"\
		"fdr(ns):Avg=$AVG_FUN Var=$VAR_FUN  Min=$MIN Max=$MAX ${TITLE}_`uname -r`" "Percentage" "Time (ns)"
